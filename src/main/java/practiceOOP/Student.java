...
public class Student implements Comparable<Student> { // compares students based on name
    ...
    public int compareTo(Student otherStudent) {
        ...
        if(this.name.compareTo(otherStudent.name)  == 0){
            return this.id < otherStudent.id ? -1 : 1;      //I assume id is no dublicate
        }


        return this.name.compareTo(otherStudent.name); // remember to change this
    }
}
